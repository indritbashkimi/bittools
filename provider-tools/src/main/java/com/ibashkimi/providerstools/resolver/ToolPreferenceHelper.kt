...
import com.ibashkimi.providerstools.MeasureUnit
...
class ToolPreferenceHelper(val tool: Tool, private val sharedPreferences: SharedPreferences) {
    ...
    fun getDisplayParams(context: Context): DisplayParams {
        return when (tool) {
            Tool.ACCELEROMETER -> DisplayParams(decimalFormat = "#")
                .applyMeasurementUnit(context, measurementUnit)
            Tool.BAROMETER -> DisplayParams(decimalFormat = "#")
                .applyMeasurementUnit(context, measurementUnit)
            Tool.COMPASS -> DisplayParams(decimalFormat = "#")
                .applyMeasurementUnit(context, measurementUnit)
            Tool.HYGROMETER -> DisplayParams(decimalFormat = "#.#")
                .applyMeasurementUnit(context, measurementUnit)
            Tool.LEVEL -> DisplayParams(decimalFormat = "#.#")
                .applyMeasurementUnit(context, measurementUnit)
            Tool.LIGHT -> DisplayParams(decimalFormat = "#")
                .applyMeasurementUnit(context, measurementUnit)
            Tool.MAGNETOMETER -> DisplayParams(decimalFormat = "#")
                .applyMeasurementUnit(context, measurementUnit)
            Tool.THERMOMETER -> DisplayParams(decimalFormat = "#.#")
                .applyMeasurementUnit(context, measurementUnit)
            else -> throw IllegalArgumentException("${tool.name} is not a provider tool")
        }
    }
...
fun Tool.withPreferences(preferences: SharedPreferences) = ToolPreferenceHelper(this, preferences)
...
fun Tool.getProvider(
    context: Context,
    helper: ToolPreferenceHelper = this.helper(context)
): Provider {
    return ProviderResolver.resolve(
        context,
        this,
        helper.providerSamplingRate,
        helper.measurementUnit.dataProcessor
    )
}
